...
floci is a suite of open-source, MIT-licensed cloud emulators built with [Quarkus Native](https://quarkus.io/guides/building-native-image). Quarkus Native compiles to a standalone binary — no JVM warmup — which is why floci starts in **24 milliseconds** and sits at **13 MiB at idle**.

There are three products today:

- **floci** — 47 AWS services on port 4566, S3-compatible, drop-in LocalStack replacement
- **floci-az** — 8 Azure services (Blob, Queue, Table, Functions, App Configuration, Key Vault, Event Hubs, Service Bus) on port 4577
- **floci-gcp** — Google Cloud Storage, Pub/Sub, and Firestore
...
Service coverage is expanding across all three clouds. The roadmap is public on GitHub — open an issue with the service you need most.
